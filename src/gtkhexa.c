...
/**
 * Paints widget borders
 */
static void paint_borders(GtkWidget *widget, GdkRectangle *area)
{
    gint line_view_size = 0;
    gint hexa_view_size = 0;
    gint char_view_size = 0;

    line_view_size = (gint) ((widget->allocation.width-10) * 0.10);
    ...
    gtk_paint_box(widget->style, widget->window, GTK_STATE_NORMAL,
                  GTK_SHADOW_OUT, area, widget, NULL,
                  0, 0,
                  line_view_size, widget->allocation.height);

    gtk_paint_box(widget->style, widget->window, GTK_STATE_NORMAL,
                  GTK_SHADOW_OUT, area, widget, NULL,
                  line_view_size + 5, 0,
                  hexa_view_size, widget->allocation.height);

    gtk_paint_box(widget->style, widget->window, GTK_STATE_NORMAL,
                  GTK_SHADOW_OUT, area, widget, NULL,
                  line_view_size + 5 + hexa_view_size + 5, 0,
                  char_view_size, widget->allocation.height);
}
